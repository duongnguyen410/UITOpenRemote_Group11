...
public class Wa1_helper extends SQLiteOpenHelper {
    private static final int DATABASE_VERSION = 1;

    // Asset table name
    private static final String TABLE_ASSET = "WEATHER";
    ...
    public Wa1_helper(Context context) {
    ...
    @Override
    public void onUpgrade(SQLiteDatabase sqLiteDatabase, int i, int i1) {// Drop older table if existed
    ...
    public void deleteAssetData(String date){
        SQLiteDatabase db = this.getWritableDatabase();
        db.execSQL("DELETE FROM WEATHER WHERE Date=?", new String[]{date});
    }
